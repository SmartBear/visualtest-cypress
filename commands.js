...
let takeScreenshot = (element, name, modifiedOptions, win) => {
    ...
    if (!vtConfFile.fail) {
        ...
        if (!modifiedOptions.lazyload && runFreezePage) { //run freeze-page script unless it is explicitly turned off
    ...
    if (vtConfFile.fail) {
        ...
        if (numScrolls <= 1) {
            ...
            if ((modifiedOptions.lazyload !== undefined) && (modifiedOptions.lazyload > 10000 || modifiedOptions.lazyload < 0 || isNaN(modifiedOptions.lazyload))) {
                ...
                cy.wait(1000);
                if (runFreezePage) {
                    cy.task('logger', {type: 'debug', message: `running freezePage after the lazyload function.`});
                    freezePageResult = win.eval(toolkitScripts.freezePage);
                    cy.task('logger', {
                        type: 'debug',
                        message: `freezePageResult: ${freezePageResult ? JSON.stringify(freezePageResult) : 'UNDEFINED'}`
                    });
                }
